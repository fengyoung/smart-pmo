...
claude pmo-cross-process --file <路径> --projects A,B --asr-correction ./custom-asr.json   # 覆盖默认 ~/.smart-pmo/asr-correction.json
...
**状态输出：**
```
📖 已加载 ASR校正表（~/.smart-pmo/asr-correction.json，{N}条）、成员映射（{M}人，跨{N}个项目）、历史会议参考（{N}条）
```
...
3. 在确认界面输出，询问用户：
   ...
   [是] [否] [选择追加]
4. 用户确认「是」或「选择追加」后，追加到本地 `~/.smart-pmo/asr-correction.json`：
   a. 读取 JSON，检查去重（以 `incorrect` + `correct` 组合为键）
   b. 新增条目格式：
      ```json
      {"incorrect": "{误识别写法}", "correct": "{正确写法}", "type": "person|term|nickname", "note": "", "addedAt": "{当前日期}"}
      ```
   c. 原子写入（先写 .tmp 再 rename）
5. 更新完成后在终端输出：✅ ASR 校正表已更新（{N}条，含术语{K}条）
6. 提示用户：💡 建议定期执行 git commit 推送变更（如被 git 管理）
```
...
For each project_id with extracted content:
  ...
  ⑤ 批量写入待办（使用 batch-create）
     ...
     - 负责人字段只接受单用户：
       ✅ [{"id": "ou_xxx", "name": "姓名"}]（单用户）
       ❌ [{"id": "ou_A"}, {"id": "ou_B"}]（多用户会报错，且缺少 name 字段也会触发 1254066）
     ...
     - 用 +record-batch-create 一次写入多条：
       lark-cli base +record-batch-create \
         ...
         --json '{
           ...
           "rows":[
             ["待办1","会议:多项目周会",[{"id":"ou_xxx","name":"张三"}],"2026-06-25","待处理","P0-紧急",[{"id":"rec_meeting"}]],
             ["待办2","会议:多项目周会",[{"id":"ou_yyy","name":"李四"}],null,"待处理","P1-重要",[{"id":"rec_meeting"}]]
           ]
         }'
     ...
     - 同样使用 +record-batch-create
       lark-cli base +record-batch-create \
         ...
         --json '{
           ...
           "rows":[
             ["M1","描述",[{"id":"ou_xxx","name":"张三"}],"未开始","2026-06-29"],
...
> 📋 卡片消息构造规范详见 [`_shared/feishu-card-template.md`](../_shared/feishu-card-template.md)。以下为操作指南，模板细节以共享模块为准。
