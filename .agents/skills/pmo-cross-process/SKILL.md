...
version: 1.8.0
description: "跨项目会议处理——一次提取多项目分发，从含多项目讨论的转写文件中提取纪要/待办/里程碑并分发到各项目（v1.8.0：新增动态项目检测、ASR校正反馈、提取详细度强化）"
metadata:
  requires:
    bins: []
  depends_on:
...
### Step 2.5b：ASR 校正反馈（v1.8.1+）

> 📝 AI 提取完成后，将提取过程中发现的疑似 ASR 错误反馈给用户，并更新 ASR 校正表。
...
3. 在确认界面输出，询问用户：
   ...
   [是] [否] [选择追加]
4. 用户确认「是」或「选择追加」后：
   a. 对于已有 ASR 校正表的项目 → 通过 lark-doc +update --command append 追加到末尾
   b. 对于尚无 ASR 校正表的项目 → 创建文档后通过 lark-wiki +move 归档到 05-项目资料/
   c. 追加格式：| {误识别写法} | {正确写法} |
5. 更新完成后在终端输出：✅ ASR 校正表已更新（{N}条）
```
