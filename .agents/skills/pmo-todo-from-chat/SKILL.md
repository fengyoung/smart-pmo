...
2. **写入结果后，按群独立更新 readPosition**：
   ...
   - 该群有待办写入但**部分失败** → **同样更新 readPosition**（已成功写入的去重机制保证不重复），但在终端标注 `⚠️ {群名} 有 {N} 条写入失败`
   - 该群**全部待办写入失败** → 不更新该群 readPosition（下次执行时重读该批消息重试）
   - 该群**无选中的待办**（用户未选该群消息）→ 不更新该群 readPosition（该批消息下次继续可见）

   **防止重复录入说明：** 由于去重检查（L2 语义相似度 + L3 用户确认），即使因为更新了 readPosition 导致下次仍读到同一批消息，去重机制会过滤掉已写入的待办。
...
| 未提取到待办 | 提示"未识别到新待办" |
| 连接失败 | 提示稍后重试 |
| 某群 readPosition 更新失败 | 警告提示，不影响其他群；下次执行时该群会重读部分消息，去重机制处理 |
