...
> 项目版本：见根目录 `VERSION` 文件（当前 v1.9.3）
...
│   └── asr-correction-table-template.md # ASR 校正表模板（v1.9.0 已迁移至 ~/.smart-pmo/asr-correction.json）
...
> ⚠️ **强制执行**：所有批量处理语音转写文件的场景（如处理多个历史会议纪要），必须按以下流程执行，**禁止跳过 AI 提取环节**。

**正确流程（每个文件）：**
```
1. cat 读取全文（必须完整，不能只读头尾）
2. Agent + Claude 完整 AI 提取：
   - 按议题分组 × 4 维度（概述、关键数据/进展、决策/结论、风险/问题）
   - 议题必须包含具体数据（数字、比例、金额），不能笼统概括
   - 待办必须有明确的负责人和截止时间
3. 用提取结果创建/更新飞书文档（docs +update --command overwrite）
4. 同步更新 Base 会议索引的讨论要点摘要和关键决策
```

**禁止：**
- ❌ 用 Python 脚本手动拼接纪要内容
- ❌ 仅基于文件名或头尾内容猜测会议内容后直接写入
- ❌ 跳过 AI 提取环节直接创建文档

**原因：** 语音转写文件通常 500-1200 行、25-60KB，包含大量具体的业务讨论和数据。不读全文直接概括会导致丢失关键数据、误判议题、待办不准确。历史教训：第一次批量处理 13 个文件时手动概括，被用户指出质量远低于 Iris；改用 Agent 读全文提取后质量达标。
...
| **v1.9.1 优化** | 全面审查与优化（P0-P2）：修复用户字段格式错误（防止 1254066）、统一 feishu-card-template 引用（8 个 Skill）、修复 depends_on 声明（3 处）、消除内联重复内容（3 个 Skill）、修复共享模块引用路径（2 处）、补全配置校验、验证脚本新增 7 项自动检查 | ✅ 已完成 |
| **v1.9.2 优化** | 规范优化：清理 9 个临时文件、修复 4 个 Skill 硬编码目录名（改为从 config.larkResources.wikiNodeTokens 动态读取）、12 个 Skill 版本号按 git diff 实证对齐至 v1.6.3 | ✅ 已完成 |
| **v1.9.3 优化** | 安装规范修复：Skill 从用户级迁移至项目级 `.claude/skills/`；6 个缺失 symlink 补齐；2 个冗余目录清理；全部 symlink 转为相对路径并纳入 git 跟踪；setup.sh 适配项目级安装 | ✅ 已完成 |
